...
3. **User Events**: Connects buttons (like "Launch" and "Manage Engines") to the asynchronous logic in `ogl-executor` and `ogl-core::engine_manager`.

## Architecture

GTK4 operates on an event-driven main loop. Heavy workloads (like engine downloads/extraction via `ogl-core::engine_manager` or waiting for the `ogl-executor` process) must be sent to separate `tokio` threadpools to avoid freezing the graphical interface. 
